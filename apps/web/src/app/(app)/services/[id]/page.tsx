...
const SERVICE_STATUSES = [
  ...
  { value: 'AWAITING_FILING', label: 'Awaiting Filing' },
...
export default function ServiceDetailPage() {
  ...
  const handleGenerateTasks = async () => {
    ...
    try {
      const result = await api.post<Task[]>(`/tasks/generate/service/${serviceId}`);
      
      // Refresh the service details to show the newly generated tasks
      await fetchService();
      
      // Show success message
      alert(`Successfully generated ${Array.isArray(result) ? result.length : 0} task(s) from template`);
    } catch (err) {
  ...
  const getStatusBadgeVariant = (status: string) => {
    switch (status) {
      ...
      case 'AWAITING_FILING': return 'warning';
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <MDJBadge
                        variant={
                          service.status === 'ACTIVE' || service.status === 'COMPLETE'
                            ? 'success'
                            : service.status === 'AWAITING_FILING' || service.status === 'READY_TO_CLOSE'
                            ? 'warning'
                            : 'default'
                        }
                      >
                      ...
                      {tasks.map((task) => (
                        <tr key={task.id} className="border-b" style={{ borderColor: 'var(--border)' }}>
                          ...
                          <td className="py-3 px-3">
                            <MDJBadge
                              variant={
                                (task.status || 'TODO').includes('COMPLETED')
                                  ? 'success'
                                  : (task.status || 'TODO').includes('CANCELLED')
                                  ? 'error'
                                  : 'info'
                              }
                            >
