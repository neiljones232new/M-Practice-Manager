  ...
  const fetchTask = async () => {
    try {
      ...
      setFormData({
        ...
        assigneeId: taskData.assignee || (taskData as any).assigneeId || '',
      ...
      if (taskData.serviceId) {
        ...
        if (serviceData) {
          ...
          if (serviceData.clientId) {
            const clientData = await api.get<Client>(`/clients/${serviceData.clientId}`).catch(() => null);
            if (clientData) {
                ...
                onCSV={async () => {
                  try {
                    const csv = await api.get<string>(
                      `/tasks/export.csv?serviceId=${encodeURIComponent(task.serviceId || '')}`,
                    );
                ...
                onXLSX={async () => {
                  try {
                    const data = await api.get<any>(
                      `/tasks/export.xlsx?serviceId=${encodeURIComponent(task.serviceId || '')}`,
                    );
                  ...
                  <MDJButton
                    ...
                    onClick={() => {
                      ...
                      setFormData({
                        ...
                        assigneeId: task.assignee || (task as any).assigneeId || '',
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm" style={{ color: 'var(--dim-light)' }}>
                        {task.assignee || (task as any).assigneeId || 'Unassigned'}
                      </p>
                    </div>
                  </div>
            ...
            <MDJCard title="Actions" padding="md">
              <div className="space-y-2">
                <MDJButton
                  ...
                  onClick={() =>
                    router.push(
                      `/tasks/new?clientId=${client?.id || ''}&serviceId=${task.serviceId || ''}`,
                    )
                  }
                >
                  Create Similar Task
