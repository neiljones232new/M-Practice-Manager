...
export default function CompliancePage() {
  ...
  const loadComplianceData = async () => {
    try {
      ...
      const serviceRows =
        services.length > 0 ? services : (((await api.get('/services/with-client-details')) as any[]) || []);
      const servicesById = new Map(serviceRows.map((service: any) => [service.id, service]));
      const now = new Date();
      ...
      console.log(
        ...
        items.slice(0, 3).map((i) => ({ id: i.id, serviceId: i.serviceId, type: i.type })),
      );
      ...
      // Enrich items with client information from the linked client service.
      items = items.map(item => {
        const service = servicesById.get(item.serviceId);
        const resolvedClientId = item.clientId || service?.clientId;
        let client = resolvedClientId ? clientByIdMap.get(resolvedClientId) : undefined;
        if (!client) {
          const fallbackRef = service?.clientIdentifier || resolvedClientId;
          if (fallbackRef) {
            client = clientByRefMap.get(fallbackRef);
          }
        }
        ...
        if (client) {
          return {
            ...item,
            clientId: client.node.id,
            description: `${client.node.name} (${client.node.registeredNumber || client.node.id}) - ${item.type.replace(/_/g, ' ')}`,
          };
        } else {
          console.warn('Client not found for compliance item service:', item.serviceId, 'Item:', item.id);
          // Still show the item but with a generic description
          return {
            ...item,
            description: `Service ${item.serviceId} - ${item.type.replace(/_/g, ' ')}`,
          };
        }
      });
