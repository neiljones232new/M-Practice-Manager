...
type Freq = 'ANNUAL' | 'QUARTERLY' | 'MONTHLY' | 'WEEKLY' | 'ONE_OFF';
...
const DEFAULT_SERVICES: ServiceInput[] = [
  { kind: 'Annual Accounts', frequency: 'ANNUAL', fee: 600, status: 'DRAFT', enabled: true },
  { kind: 'Corporation Tax Return', frequency: 'ANNUAL', fee: 250, status: 'DRAFT', enabled: true },
  { kind: 'Company Secretarial', frequency: 'ANNUAL', fee: 60, status: 'DRAFT', enabled: true },
  { kind: 'Payroll Services', frequency: 'MONTHLY', fee: 100, status: 'DRAFT', enabled: true },
  { kind: 'VAT Returns', frequency: 'QUARTERLY', fee: 120, status: 'DRAFT', enabled: true },
  { kind: 'Self Assessment', frequency: 'ANNUAL', fee: 350, status: 'DRAFT', enabled: true },
];
...
export default function AddClientWizardPage() {
  ...
  const addService = () => setServices((prev) => ([...prev, { kind: '', frequency: 'ANNUAL', fee: 0, status: 'DRAFT', enabled: true }]));
  ...
  const submit = async () => {
    try {
      ...
      router.push(id ? `/clients/${id}/overview` : '/clients');
    } catch (e: any) {
              ...
              <tbody>
                {services.map((s, i) => (
                  <tr key={i}>
                    ...
                    <td>
                      <select className="input-mdj" value={s.status || 'DRAFT'} onChange={(e)=>updateService(i, { status: e.target.value as any })}>
                        <option value="DRAFT">Draft</option>
                        <option value="ACTIVE">Active</option>
                        <option value="AWAITING_FILING">Awaiting Filing</option>
