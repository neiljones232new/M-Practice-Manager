...
export default function ClientPartiesPage() {
  ...
  const saveNewParty = async () => {
    ...
    try {
      ...
      await api.post(`/clients/${clientId}/parties`, {
        personId: person.id,
        role: partyForm.role,
  ...
  const saveEditParty = async () => {
    ...
    try {
      ...
      await api.put(`/clients/${clientId}/parties/${selectedParty.id}`, {
  ...
  const handleSetPrimary = async (party: ClientParty) => {
    ...
    setError(null);
    try {
      await api.put(`/clients/${clientId}/parties/${party.id}`, { primaryContact: true });
      await loadClient();
    } catch (e: any) {
  ...
  const handleResign = async (party: ClientParty) => {
    ...
    setError(null);
    try {
      await api.put(`/clients/${clientId}/parties/${party.id}`, { resignedAt: new Date().toISOString() });
      await loadClient();
    } catch (e: any) {
  ...
  const handleDelete = async (party: ClientParty) => {
    ...
    setError(null);
    try {
      await api.delete(`/clients/${clientId}/parties/${party.id}`);
      await loadClient();
    } catch (e: any) {
