...
export default function ClientsSummaryPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((c) => {
      const node = c.node;
      const txt = `${node.name} ${node.clientRef || ''} ${node.registeredNumber || ''} ${node.utrNumber || ''}`.toLowerCase();
  ...
  const portfolioOptions = useMemo(() => {
    ...
    for (const row of rows) {
      const code = row?.node?.portfolioCode;
      if (typeof code === 'number' && Number.isFinite(code)) {
        uniqueCodes.add(code);
      }
    }
    return Array.from(uniqueCodes).sort((a, b) => a - b);
  }, [rows]);
        ...
        {loading ? (
          ...
          <table className="mdj-table">
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.node.id}>
                  <td><span className="gold-ref">{c.node.clientRef || '—'}</span></td>
                  <td>{c.node.status}</td>
                  <td>{c.node.name}</td>
