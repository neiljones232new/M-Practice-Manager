...
const resolveServiceRef = (value?: { serviceId?: string } | null): string | null => {
...
const isValidWorkTab = (value?: string | null): value is WorkTab =>
  value === 'services' ||
  value === 'accounts' ||
  value === 'tax' ||
  value === 'tasks' ||
...
export default function ClientDetailsContent({
  ...
  initialPrimaryTab: PrimaryTab;
}) {
  const router = useRouter();
  const params = useParams<{ id?: string }>();
  const searchParams = useSearchParams();
  const clientId = String(params?.id || '').trim();
  const hasValidClientId = Boolean(clientId && clientId !== 'undefined' && clientId !== 'null');
  ...
  const clientIdentifier = client?.clientRef || client?.id || '';
  const clientRouteId = (() => {
  ...
  const clientLookupId = (() => {
    const candidates = [client?.id, clientId];
    for (const raw of candidates) {
      const value = String(raw || '').trim();
      if (!value || value === 'undefined' || value === 'null') continue;
      return value;
  ...
  const workTabParam = searchParams?.get('workTab');
  const [workTab, setWorkTab] = useState<WorkTab>(isValidWorkTab(workTabParam) ? workTabParam : 'services');
  const primaryTab = useMemo<PrimaryTab>(() => {
    const candidate = initialPrimaryTab;
    if (candidate === 'companies' && !showCompaniesHouse) return 'overview';
    return candidate;
  }, [initialPrimaryTab, showCompaniesHouse]);
  ...
  useEffect(() => {
    if (!isValidWorkTab(workTabParam)) return;
    setWorkTab(workTabParam);
  }, [workTabParam]);
  ...
  }, [clientId, hasValidClientId, refreshTick]);
  ...
  const handleGenerateLetter = async () => {
    if (!clientLookupId || !letterTemplateId) return;
    ...
    try {
      ...
      await api.post('/letters/generate', {
        ...
        clientId: clientLookupId,
        serviceId: letterServiceId || undefined,
        outputFormats,
        autoSave: true,
      });
      await refreshLetters(clientLookupId);
      await refreshDocuments(clientLookupId);
  ...
  const handleSync = async () => {
    if (!clientLookupId) return;
    setSyncMessage(null);
    setSyncing(true);
    try {
      const res = await api.post<{ message?: string }>(`/companies-house/sync/${clientLookupId}`);
      setSyncMessage({ text: res?.message || 'Synchronized with Companies House', error: false });
      const refreshed = await api.get<ClientContextWithParties>(`/clients/${clientLookupId}/with-parties`).catch(() => null);
      if (refreshed) {
  ...
  const handleGenerateTasks = async () => {
    ...
    try {
      const res = await api.post<{ created: number; services: number }>(
        `/tasks/generate/client/${clientLookupId}`
      );

      const totalTasks = Number(res?.created || 0);
      const servicesProcessed = Number(res?.services || 0);
      const message =
        totalTasks === 0
          ? 'No new tasks were created; services may already have open tasks or are not active.'
          : `Generated ${totalTasks} task${totalTasks === 1 ? '' : 's'} across ${servicesProcessed} service${servicesProcessed === 1 ? '' : 's'}.`;

      setTaskMessage({ text: message, error: false });
      if (clientLookupId) {
        const refreshedServices = await api.get<Service[]>(`/clients/${clientLookupId}/services`).catch(() => []);
  ...
  const handleEnrollDirector = async (directorName: string) => {
    ...
    try {
      ...
      if (nextId) {
        router.push(`/clients/${nextId}/overview`);
      }
    } finally {
            ...
            {primaryTabKeys.map((key) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    const targetId = clientRouteId;
                    if (!targetId) return;
                    if (key === 'overview') {
                      router.push(`/clients/${encodeURIComponent(targetId)}/overview`);
                      return;
                    }
                    if (key === 'work') {
                      router.push(`/clients/${encodeURIComponent(targetId)}/work`);
                      return;
                    }
                    router.push(`/clients/${encodeURIComponent(targetId)}/companies`);
                  }}
                  style={{
                    padding: '12px 16px',
                    border: 'none',
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (!clientLookupId) return;
                      router.push(`/accounts-production/new?clientId=${encodeURIComponent(clientLookupId)}`);
                    }}
                  >
                    Accounts Production
                  </button>
          ...
          <div style={{ padding: '1rem' }}>
            <div style={{ marginBottom: '0.75rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <button
                ...
                onClick={() => {
                  if (!clientLookupId) return;
                  router.push(`/tax-calculations/new?clientId=${encodeURIComponent(clientLookupId)}`);
                }}
              >
                New Tax Calculation
              </button>
            ...
            {tasks.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem' }}>
                ...
                <button
                  ...
                  onClick={() => {
                    if (!clientLookupId) return;
                    router.push(`/tasks/new?clientId=${encodeURIComponent(clientLookupId)}`);
                  }}
